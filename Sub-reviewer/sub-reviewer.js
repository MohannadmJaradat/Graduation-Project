...
document.getElementById('cond').addEventListener('click', function() {
    localStorage.setItem("where","Sub-reviewer")
});
...
async function fetchsubmissions() {
        ...
        const conferenceHTML = `
            <div class="col-md-12 col-lg-6 mx-auto gap-5" data-submission-id="${submission._id}">
                <div class="card test-card" style="min-width: 18rem;">
                    ...
                    <div class="card-body">
                        <p class="card-text">Author name: ${user.fullName}</p>
                        ...
                        <div class="d-flex col-12 mx-auto mt-2">
                            <button type="button" class="btn save-and-continue-btn col-5 mx-auto paper" style="margin-top: 3%;" data-paper-url="http://localhost:3000/${submission.paper}">Download Paper</button>
                            <button type="button" class="btn btn-warning col-5 mx-auto abstract" style="margin-top: 3%;" data-abstract-url="http://localhost:3000/${submission.abstract}">Download Abstract</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const abstractButtons = document.querySelectorAll('.abstract');
    abstractButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            event.preventDefault(); // Prevent any default behavior
            const abstractUrl = button.getAttribute('data-abstract-url');
            if (abstractUrl) {
    ...
    button.addEventListener('click', async (event) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                window.location.href="./Sub-reviewer.html"
            } else {
